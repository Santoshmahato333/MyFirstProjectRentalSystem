        ...
        if(mysqli_num_rows($query)>0)
        {
          while($row=mysqli_fetch_assoc($query)){ ?>
                ...
                ?>
                    ...
                    <td>
                      <form method="post"><input type="text" name="pid" style="display: none;" value="<?php echo $rows['property_id'] ?>"><button class="btn btn-danger" onclick="if (!confirm('Are you sure you want to delete the property?\n\nThis action cannot be undone!')) {event.preventDefault();}" name="delete_property">Delete</button></form>
                        <br>
                        <form method="get" action="./update-property.php?pid=<?=$rows['property_id']?>"><input type="text" name="pid" style="display: none;" value="<?php echo $rows['property_id'] ?>"><button class="btn btn-primary" onclick="if (!confirm('Are you sure you want to update the property?')) {event.preventDefault();}">Update</button></form>
                    </td>
              <!-- <td><a href="delete_property.php?pid=<?=$rows['property_id']?>&">Delete</a></td> -->
              <?php
                }}}} ?>
              </tr>
            </table> 
...
<div id="menu6" class="tab-pane fade">
      ...
      <?php 
        ...
        $sql3="
               select
                     booking.bid, booking.tid, booking.pid,
                       tenant.full_name, tenant.address, ap.province,
                             ap.zone, ap.district, ap.zone, ap.ward_no,
                              ap.tole from booking 
                              join add_property as ap 
                              on ap.property_id = booking.pid 
                              join owner 
                              on owner.owner_id = ap.owner_id 
                              join tenant on tenant.tenant_id = booking.tid 
                              where booking.pid in
                               (select property_id from add_property 
                                    join owner on owner.owner_id = add_property.owner_id
                                     where email = '$u_email')  and (CURRENT_TIMESTAMP - booking.start_date) < 86400;";
              //  echo $sql3;
        ...
        <td>
        <center><form method="POST"><input name="bid" value="<?=$row['bid']?>" hidden><button id="submit" onclick="if (!confirm('Are you sure you want to cancel this booking?')) {event.preventDefault();}" name="cancel_booking" class="btn btn-danger btn-block">Cancel Booking</button></form></center><br>
              ...
              echo "<center><td colspan=\"8\" style=\"text-align: center;\"><b><i>You donot have booked any property yet!</i></b></td></center>";
            }
            ?>
                <tr>
